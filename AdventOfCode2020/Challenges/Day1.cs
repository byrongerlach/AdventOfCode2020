...
namespace Challenges
{
    ...
    public class Day1
    {
        ...
        public static int FindProductOfTwo(IEnumerable<int> numbers, int targetSumValue)
        {
        ...
        /// <summary>
        // Find the product of three numbers that sum to targetSumValue.
        /// </summary>
        /// <param name="numbers">The numbers.</param>
        /// <param name="targetSumValue">The target sum value.</param>
        /// <returns></returns>
        public static int FindProductOfThree(IEnumerable<int> numbers, int targetSumValue)
        {
            // Sort so we can skip numbers that are too large
            var sortedNumbers = numbers.OrderByDescending(n => n).ToArray();
            ...
            for (var i = 0; i < sortedNumbers.Length; i++)
            {
                ...
                for (var j = i + 1; j < sortedNumbers.Length; j++)
                {
                    for (var k = j + 1; k < sortedNumbers.Length; k++)
                    {
                        if (sortedNumbers[i] + sortedNumbers[j] + sortedNumbers[k] == targetSumValue)
                        {
